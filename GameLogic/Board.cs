...
namespace GameLogic
{
    public class Board
    {
        ...
        private void Initialize(int width, int height, int side, HexOrientation orientation, int xOffset, int yOffset)
        {
            ...
            hexes = new Hex[width, height]; //opposite of what we'd expect
            this.boardState = new BoardState();

            float h = MathClass.CalculateH(side); // short side
            float r = MathClass.CalculateR(side); // long side
                    ...
                    if (isTopLeft)
                    {
                        ...
                        switch (orientation)
                        {
                            ...
                            case HexOrientation.Pointy:
                                ...
                                hexes[0, 0] = new Hex(0 + h + xOffset+20, 0 + yOffset, side, orientation, j, i); //BL //added +10 to xOffset
                                break;
                            default:
                                break;
                        }
                        ...
                        switch (orientation)
                        {
                            case HexOrientation.Flat:
                                if (inLeftColumn)
                                {
                                    ...
                                    hexes[j, i] = new Hex(hexes[i - 1, j].Points[(int)FlatVertice.BottomLeft], side, orientation);
                                }
                                else
                                {
                                    ...
                                    if (j % 2 == 0)
                                    {
                                        ...
                                        float x = hexes[j - 1, i].Points[(int)FlatVertice.UpperRight].X;
                                        float y = hexes[j - 1, i].Points[(int)FlatVertice.UpperRight].Y;
                                        x += h;
                                        y -= r;
                                        hexes[j, i] = new Hex(x, y, side, orientation);
                                    }
                                    else
                                    {
                                        // Calculate from Hex to left's Middle Right Vertice
                                        hexes[j, i] = new Hex(hexes[j - 1, i].Points[(int)FlatVertice.MiddleRight], side, orientation);
                                    }
                                }
                                break;
                            case HexOrientation.Pointy:
                                if (inLeftColumn)
                                {
                                    ...
                                    if (i % 2 == 0)
                                    {
                                        ...
                                        hexes[j, i] = new Hex(hexes[j, i - 1].Points[(int)PointyVertice.BottomLeft], side, orientation, j, i); //BL
                                    }
                                    else
                                    {
                                        //hexes[i, j] = new Hex(hexes[i - 1, j].Points[(int)PointyVertice.BottomRight], side, orientation);
                                        hexes[j, i] = new Hex(hexes[j, i - 1].Points[(int)PointyVertice.BottomRight], side, orientation, j, i); //BL
                                    }
                                ...
                                else
                                {
                                    ...
                                    float x = hexes[j - 1, i].Points[(int)PointyVertice.UpperRight].X;
                                    float y = hexes[j - 1, i].Points[(int)PointyVertice.UpperRight].Y;
                                    x += r;
                                    y -= h;
                                    //hexes[i, j] = new Hex(x, y, side, orientation);
                                    hexes[j, i] = new Hex(x, y, side, orientation, j, i); //BL
                                }
                                break;
                            default:
                                break;
        ...
        public Hex FindHexMouseClick(int x, int y)
        {
            ...
            if (PointInBoardRectangle(x, y))
            {
                for (int i = 0; i < hexes.GetLength(0); i++)
                {
                    ...
                    {
                        if (MathClass.InsidePolygon(hexes[j, i].Points, 6, new System.Drawing.PointF(x, y)))
                        {
                            target = hexes[j, i];
                            break;
                        }
